...
function RealisticWeatherLite:update(dt)
    ...
    ---------------------------------------------------------------------------
    -- GRANDINE: USURA E DANNI AI VEICOLI
    ---------------------------------------------------------------------------
                ...
                for _, vehicle in pairs(vehicles) do
                    if vehicle.spec_wearable ~= nil and vehicle.rootNode ~= nil then
                        ...
                        if not isIndoor then
                            -- Valori bilanciati per applicare sia usura (vernice) che danno meccanico
                            local damageAmount = hail * 0.0015 * (dt / 16.67)
                            local wearAmount = hail * 0.0035 * (dt / 16.67)
