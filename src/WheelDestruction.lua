...
function RW_WheelDestruction:destroySnowArea(superFunc, x0, z0, x1, z1, x2, z2)
    -- Sicurezza: Controlla che self, self.vehicle e self.wheel siano validi
    if self == nil or self.vehicle == nil or self.wheel == nil then 
        return superFunc(self, x0, z0, x1, z1, x2, z2) 
    end

    local snowSystem = g_currentMission.snowSystem
    if snowSystem == nil then return superFunc(self, x0, z0, x1, z1, x2, z2) end
    
    local snowHeight = snowSystem:getSnowHeightAtArea(x0, z0, x1, z1, x2, z2)
    local sinkHeight

    if self.ticksSinceLastDestruction == nil then 
        self.ticksSinceLastDestruction = RW_WheelDestruction.TICKS_PER_DESTRUCTION 
    end
    ...
    if SnowSystem.MIN_LAYER_HEIGHT < snowHeight and self.ticksSinceLastDestruction >= RW_WheelDestruction.TICKS_PER_DESTRUCTION then
        local mass = self.vehicle:getTotalMass() or 5
        local wheels = 0

        if self.vehicle.spec_wheels ~= nil and self.vehicle.spec_wheels.wheels ~= nil then 
            wheels = #self.vehicle.spec_wheels.wheels 
        end

        local wheelsFull = self.vehicle.spec_wheels.wheels or {}
            ...
            if wheel.visualWheels ~= nil and #wheel.visualWheels > 1 then
                ...
                if wheel == self.wheel then
                    for i, visualWheel in pairs(wheel.visualWheels) do
                        if i ~= 1 then -- Sostituito il continue illegale
        ...
        if g_currentMission.environment ~= nil and g_currentMission.environment.weather ~= nil then
            groundWetness = g_currentMission.environment.weather:getGroundWetness() or 0
        end

        minSinkHeight = math.max(minSinkHeight * (1 - groundWetness), minSinkHeight * 0.75)
        sinkHeight = math.max(sinkHeight, minSinkHeight * (snowSystem.height or 1))
